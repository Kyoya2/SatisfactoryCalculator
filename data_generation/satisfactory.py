...
class Recipe(NamedTuple):
    name: str
    products: CountedItems # minify frac
    ingredients: CountedItems # minify frac
    duration: Fraction # minify
    is_alternate: bool  # minify
    produced_in: set[str]
...
class SatisfactoryParser:
    ...
    def generate_data_file_content(self):
        ...
        crafting_objects_map = list(sorted(self._crafting_objects))
        recipes_map = list(sorted(self._recipes))

        minified_crafting_objects = []
        for i, crafting_obj_id in enumerate(crafting_objects_map):
            jsonified = jsonify(self._crafting_objects[crafting_obj_id])
            jsonified['id'] = i
            jsonified['recipes'] = [recipes_map.index(recipe_id) for recipe_id in jsonified['recipes']]
            minified_crafting_objects.append(jsonified)

        minified_recipes = []
        for i, recipe_id in enumerate(recipes_map):
            jsonified = jsonify(self._recipes[recipe_id])
            ...
            # real changes.
            del jsonified['produced_in']

            for key_name in ('products', 'ingredients'):
                jsonified[key_name] = [(crafting_objects_map.index(obj_id), amount) for obj_id, amount in jsonified[key_name].items()]
            minified_recipes.append(jsonified)
